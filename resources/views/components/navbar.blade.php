...
<!-- errors -->
@if ($errors->any())
    <div class="message hidden w-1/2 md:w-3/12 bg-red-100/80 border border-red-400/20 shadow-lg shadow-red-500 text-red-700 px-4 py-3 rounded fixed z-20 right-10 top-16" role="alert">
...
@endif
@if(session('message'))
    <div class="message hidden w-1/2 md:w-3/12 bg-green-100/80 border border-red-400/20 shadow-lg shadow-green-500 text-green-700 px-4 py-3 rounded-md fixed z-20 right-10 top-16" role="alert">
    ...
    <div class="bg-white max-h-[500px] h-96 w-10/12 md:w-5/12 rounded-md shadow-md mx-auto my-auto p-4 overflow-y-auto scrollbar-small">
        ...
        <section id="name_form" class="hidden">
            <form action="{{route('update_name.user')}}" method="POST" class="p-4 rounded-md bg-slate-200 mt-1">
                ...
                <fieldset class="w-full">
                    ...
                    <label class="mr-2 font-semibold text-gray-600 inline align-middle">Change name</label>
                </fieldset>
        ...
        <section class="hidden" id="dob_form">
            <form action="{{route('update_dob.user')}}" method="POST" class="p-4  rounded-md bg-slate-200 mt-2 flex justify-between">
                @method('PUT')
                @csrf
                <label for="dob" class="mr-2 text-sm font-semibold text-gray-600 w-4/12">Change Date of Birth:</label>
                <fieldset class="flex justify-end">
